...
## Purpose
The wall-clock-minute-aligned snapshot of per-channel energy counters and voltage, published on its own dedicated MQTT topic separate from the `meter` (power) and `grid` topics. Unconditional (no boolean gate), reusing the same alignment primitive as the grid telemetry stream. A snapshot, not a queued stream: read directly at each boundary rather than buffered between boundaries.
