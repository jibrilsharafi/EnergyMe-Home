...
## Purpose
The `meter` MQTT topic's power-only payload and its threshold/interval-driven publish trigger. Energy counters and voltage are published separately by `energy-telemetry-stream`; this capability governs only the power point stream and the size estimate that decides when to flush it.
