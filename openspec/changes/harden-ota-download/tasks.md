...
- [x] 6.7 Subscribe to `jobs/+/update/rejected` (not `/update/accepted`, which would echo every progress update): a rejection is logged as ERROR with its code and message. Verified 2026-09-23 on the Pro bench (`14c19f4e1b44`): a dev job force-cancelled mid-download had both its `rebooting` IN_PROGRESS and its post-validation SUCCEEDED rejected with `InvalidStateTransition`, each logged. The device still completes a cancelled job it already started (it learns of the cancel only through these rejections); the image is signed, so this is noise, not a risk
- [x] 6.10 Heap across the full 5-attempt schedule re-measured 2026-09-23 (unresolvable host via `inject-job`, real 2/4/8/15 min backoffs, system info polled every 3 s for 35 min) on both products at HEAD. Pro (`14c19f4e1b44`, octal PSRAM): `minFreeHeap` 65 984 bytes from attempt 1 to 5, never moved; `maxAlloc` 31 732-37 876. Home (`907069875394`, quad PSRAM): `minFreeHeap` 91 436 -> 85 436, the decline coming during the 8 min backoff while the OTA task only sleeps, then flat; free heap back to ~110 KB once the task exits. No ratchet from the retry loop; the 8 128-byte reading was not reproduced on either board
...
- [x] 7.7 `_otaValidationTask` now calls `_validateOtaUpdate()` (early returns) and clears the pending state, the handle and the task in one place. Verified 2026-09-23 with a real dev job on the Pro bench: SUCCEEDED after the 300 s window
