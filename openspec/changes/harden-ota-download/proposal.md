...
- Report diagnostics on failure as discrete `statusDetails` name-value pairs rather than a single opaque reason: the failing `esp_err_t` name, the HTTP status, download progress, the internal-heap figures (download failures only), attempt count, uptime and RSSI. The `reason` key stays; it keeps `download_failed` for a generic download failure, and carries a more specific value where the device knows one (`incomplete_download` for a truncated transfer since #233, the signature and `image_incompatible:*` reasons from later changes).
