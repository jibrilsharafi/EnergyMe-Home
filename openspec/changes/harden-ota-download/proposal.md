...
- Report diagnostics on failure as discrete `statusDetails` name-value pairs rather than a single opaque reason: the failing `esp_err_t` name, the HTTP status, download progress, the internal-heap figures (retryable failures only), attempt count, uptime and RSSI. The `reason` key stays; it keeps `download_failed` for a generic download failure, and carries a more specific value where the device knows one (`incomplete_download` for a truncated transfer since #233, the signature and `image_incompatible:*` reasons from later changes).
...
### New Capabilities
- `ota-download-resilience`: how the firmware OTA download behaves under failure - retry scheduling, when retrying is abandoned, and the diagnostic detail reported with a failed job execution. (Publish quiescing during the download was implemented and then removed in 27df03a; see design.md "No publish suppression during the download".)
...
- `source/src/mqtt.cpp`: `_otaTask` retry loop, shared `OtaAttempt` state filled by `_performOtaUpdate` and `_otaHttpEventHandler`, `_publishOtaStatus` status details.
- `source/include/mqtt.h`: retry and backoff constants.
...
- Fleet-facing: `statusDetails` on a `FAILED` OTA job execution gains new keys, additively. `reason` stays `download_failed` for a generic download failure (a truncated transfer reports `incomplete_download` since #233). A `FAILED` report can now arrive up to ~44 min after the download starts instead of immediately.
