...
- [ ] 9.4 Failover tests on hardware: cable pull → STA, cable return → ETH, flap, static-IP backstop, button reset, DNS/NTP follow the active interface (verified 2026-09-19: static IP apply + reset-to-DHCP over the API, cable pull → STA in ~2 s, cable return → ETH after the 10 s hold-down, wire-only device raises the authenticated AP on link loss and drops it when the wire returns, button network reset clears WiFi + static config and keeps the password. 2026-09-20: DNS follows the active interface - with a static Ethernet config carrying DNS 1.1.1.1 and the WiFi lease bringing another server, the resolver goes to 1.1.1.1 when the wire takes the route and stays there; the static boot-fail counter clears 20 s after link-up, before a clean restart is possible. Flap and NTP-follow on a cable pull remain)
