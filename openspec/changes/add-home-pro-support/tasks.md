...
- [x] 9.6 RELEASE GATE - dual-netif heap soak: free/minFree/maxAlloc during MQTT TLS publishes and a full cloud OTA with both interfaces up (2026-09-19 progress, both interfaces up, no TLS yet: idle internal free 49.5 KB -> 89.6 KB and the floor under a 3-client web burst 22.9 KB -> 64.1 KB, from registering the web routes in PSRAM (27 KB), not running the disabled custom MQTT / InfluxDB tasks (12 KB) and sizing JSON responses once in PSRAM; the TLS MQTT + cloud OTA soak itself still needs the cloud side; 2026-09-21: a real cloud OTA job + TLS MQTT did run on this bench unit, but Ethernet-only, so it didn't count - closed properly same day: WiFi reconfigured on the Pro bench unit alongside its existing Ethernet link (confirmed both `connected:true` and `linkUp:true` simultaneously, activeInterface stayed ethernet), cloud MQTT TLS publishing meter/telemetry data continuously throughout (mqtt task ticking, staleMs~100), then a real signed cloud OTA job run against it with 1 Hz heap sampling via `/api/v1/system/info`. Idle dual-interface baseline: free 92.9 KB, minFree 70.3 KB, maxAlloc 33.8 KB. During the live TLS HTTPS download: free bottomed at 67.9 KB, the low-water minFree metric settled at 65.2 KB, maxAlloc held at 31.7 KB - comfortably clear of exhaustion. Device rebooted into the new image, both interfaces reconnected automatically, health check OK, crash archive unchanged (still 2 entries, no new one).)
