...
`DEVICE_ID` (`getDeviceId()`, `utils.cpp`) is `esp_efuse_mac_get_default()` - the factory-burned base MAC - not `WiFi.macAddress()` or `ETH.macAddress()`. This already holds for Pro with no code change: WiFi and Ethernet MACs are both *derived* from that same base - WiFi STA = base; the core's SPI-Ethernet path (`ETH.cpp`: `esp_efuse_mac_get_default()` + `esp_derive_local_mac()`) yields the base with the locally-administered bit set, NOT the IDF `base+3` scheme (bench-verified 2026-09-19: WiFi `14:C1:9F:4E:1B:44`, Ethernet `16:C1:9F:4E:1B:44`; this is the MAC a router shows and the one the label must carry) - so they differ from each other by design, but `DEVICE_ID` is independent of both and stable across interface switches, reboots with only one interface cabled/associated, and the ETH/STA failover in D5.
...
- [ACCEPTED: a static ETH config that is wrong-but-bootable (bad gateway/subnet, link+address fine) has no automatic DHCP recovery, unlike WiFi's opt-in internet-check fallback] → The boot-fail backstop covers crash/boot loops; unreachability recovers via the button long-press (network reset to DHCP). Internet-check parity deferred - EthConfiguration deliberately has no fallbackToDhcp in v1.
- [ACCEPTED: a firmware artifact with no product token (self-built, renamed) bypasses the manual-upload product gate] → Documented pass-through: unknown ≠ mismatch, community self-builds must keep working. Digest auth is required, the act is owner-initiated, and rollback/serial recovery applies. The gate is a guard-rail against OUR mis-shipped artifacts, not a control against the authenticated owner.
- [FIXED post-review: an Ethernet-commissioned Pro stayed UNPROVISIONED forever (provisioning state derives from WiFi credentials), re-arming the AP auth carve-out on any later recovery raise] → custometh persists a commissioning marker on first ETH serviceability; provisioning init treats commissioned as provisioned, so later AP raises land in AP_ASSIST (full auth). The first boot is covered too (2026-09-19): the marker is written before the WiFi task is woken, which feeds `WIRED_COMMISSIONED` and moves UNPROVISIONED → AP_ASSIST at once, so a cable pulled before the first restart raises an authenticated AP.
- [FIXED on first hardware bring-up, 2026-09-19] (a) the credential-less boot path raised the SoftAP unconditionally before Ethernet could report link, and a commissioned credential-less device sat in STA_CONNECTING forever, so after the wire-driven teardown a pulled cable meant no interface and no AP until a power cycle → `init()` starts such a device in AP_ASSIST with the AP down, `shouldRaiseAp()` owns the raise with a link-detect window (10 s) inside the DHCP grace (15 s), both counted from init, not power-on; (b) only a WiFi association released the blue network LED layer → the WiFi task follows the wire's serviceable edges; (c) USB-Serial-JTAG resets (monitor open/close, esptool) were counted as crashes and walked the rollback ladder → USB/JTAG/EXT are benign; (d) with a USB host attached and the COM port closed, dev builds spent ~280 s in the core's pre-setup chip report (100 ms HWCDC timeout per char) → timeout set from the weak `getArduinoSetupWaitTime_ms()` hook; (e) serial uploads wrote `boot_app0.bin` at the core default 0xE000, inside this table's nvs → `board_upload.arduino.boot_app0 = 0x1A000`.
- [OPEN, all products] The prebuilt libs give the IPC tasks a 1024-byte stack; `gpio_install_isr_service()` leaves ipc1 with 88 bytes free, less than an Xtensa interrupt frame, so an interrupt landing at its deepest point panics ("Stack canary watchpoint triggered (ipc1)", seen once on the bench under USB interrupt load). Mitigated by installing the service first thing in setup() with the USB IRQ masked; the tick/IPI residual can only be closed by custom libs. Watch the fleet crash archive for ipc0/ipc1 canary panics.
- [OPEN, feeds 9.6] Internal heap on the wire-only Pro in community mode, no TLS at all: ~52 KB free idle, 26.7 KB min under 4 parallel HTTP downloads (WiFi Home dev unit, WiFi + cloud, 7.5 days up: 68 KB free, 22 KB min). Same SPIRAM/WiFi/LWIP sdkconfig in qio_opi and qio_qspi, so not the 2026-08-12 regression class: it is the Home budget plus the eth task, the W5500 RX task and a WiFi driver that stays initialised with nothing to do. Candidate lever if 9.6 fails: `WiFi.mode(WIFI_OFF)` while the wire serves a credential-less device (A/B measure first).
