...
- Build: one codebase, two binaries. New `esp32s3-dev-pro`/`esp32s3-prod-pro` envs in `platformio.ini` with `qio_opi` memory type for the N16R8 module (Home envs untouched). No new lib_deps (core `ETH.h`). Manual OTA upload filename check and cloud job targeting become product-aware so a wrong-product image cannot be flashed.
- Manufacturing (coordinated, other repo): pydantic payload adds mandatory `product_line`; label keeps the base (WiFi) MAC only, the Ethernet MAC is base + 1 and is documented rather than printed; EOL test adds W5500 link test and external-antenna RF check (the antenna is mandatory: the WROOM-1U has no on-module antenna and found no network at all without it on the bench).
