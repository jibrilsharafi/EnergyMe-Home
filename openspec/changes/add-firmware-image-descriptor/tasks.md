...
- [ ] 6.2 Cloud OTA job with a wrong-product image fails with `image_incompatible:*` and the device keeps running - 2026-09-21: not yet exercised. The 7 synthetic negative-path jobs tested that day all pointed at a non-resolving URL, so none reached download or `ImageDescriptor::validate()` - they died earlier at job-document field checks (`product_mismatch`, `unknown_product`, `signature_missing_or_invalid`, etc). The one real download tested failed at mbedTLS signature verification, a stage that also runs before the descriptor check. `image_incompatible:*` has never actually fired. Still open.
