...
- Images without a descriptor (all releases before this change) remain flashable on Home devices running a quad-PSRAM image - rejecting them would break legitimate downgrades - and are rejected on Home Pro, which never shipped a descriptor-less firmware, and on any device whose running image is octal.
...
- Rollback (API/MQTT `firmware_rollback` and the crash-ladder automatic rollback) validates the passive slot's descriptor with the same policy before switching the boot partition, so a complete wrong image parked there cannot be activated by rollback.
- The running and passive-slot descriptors are reported in system info and the device shadow.
