...
**7. Re-check the verified firmware's actual embedded version against a downgrade, separate from the job document's claimed version - found during adversarial code review.**
The signature covers the firmware bytes only; the job document's `firmware.version` field (what the pre-existing `ota-job-version-guard` compares against `FIRMWARE_BUILD_VERSION`) is not part of what's signed. An attacker who replays an old, validly-signed `firmware.bin` alongside a forged job document claiming a fake higher version bypasses that guard entirely - the signature still verifies (it's genuinely signed), so the device would install a possibly-vulnerable old release under a false "upgrade" label. Fixed by re-running the same downgrade comparison inside `_verifyOtaSignature`, after cryptographic verification succeeds, against the version actually embedded in the verified bytes (`esp_ota_get_partition_description`) rather than the job document's self-reported string - skipped when `force` is set, preserving that field's existing override semantics. This is an additional, independent check; `ota-job-version-guard`'s own logic and tests are unchanged.
