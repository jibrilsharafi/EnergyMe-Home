...
- [x] 13.8 Real infra tooling exercised end to end (dev, 2026-08-14): `ota_release.py sign --env dev --from-file <bench.bin> --version 2.4.0-bench1 --profile admin-dev` (KMS-signed via `alias/energyme-home-dev-ota-signing` with the built-in verify-back, uploaded `.bin` + `.bin.sig` to `fw/2.4.0-bench1/`), then `ota_release.py create --env dev --version 2.4.0-bench1 --target thing:588c81c479f8 --profile admin-dev` (real IoT job via the `energyme-home-dev-iot-presign-role`). Offline `openssl dgst -verify` of the script-produced `.sig` against the embedded PEM → `Verified OK`; on device the job's signature `verified successfully` against the embedded official key → installed → job **SUCCEEDED**. Closes the gap that 13.6/13.7 signed with `aws kms sign` directly rather than through the shipping script. Test job + S3 objects cleaned up afterwards
...
## 15. Pre-merge code-review + simplify round (mandatory per project CLAUDE.md)

Adversarial correctness review plus a 4-angle simplify pass over the full `development...HEAD` diff, per this repo's required PR gate.

- [x] 15.1 Fixed: `_verifyOtaSignature()` failures were all treated as retry-abandoning, even transient local causes (flash read glitch, heap miss) that have nothing to do with the artifact - retrying could easily succeed. Added `_otaVerifyFailureDeterministic`, set true only for `signature_invalid`/`pubkey_parse_error` (the two causes that are actually invariant across retries of the same artifact/key); `_performOtaUpdate` now derives `_otaFailureRetryable` from it instead of hardcoding `false`
- [x] 15.2 Fixed: `_scrubRejectedOtaImage()` ran on every post-download failure with partial bytes written, including retryable ones like `incomplete_download`. A truncated image's tail is stale bytes from the previous firmware and already fails `esp_image_verify`'s checksum, so scrubbing it bought no security while costing a sector erase on every flaky-network retry - scoped the scrub call to non-retryable failures only, matching the bypass it actually guards against
- [x] 15.3 Simplify: collapsed `_performOtaUpdate`'s two `esp_https_ota_abort()` call sites (one per branch of an `if`/`else if`/`else`) into one, via a straight-line sequence of checks matching the actual pipeline order
- [x] 15.4 Simplify: replaced the hardcoded `digest[32]` with `Sha256Hex::DIGEST_LEN`, the constant task 11.1 introduced specifically to name that size
- [ ] 15.5 Not fixed, documented in the commit instead: `ota_signature.cpp`'s hand-rolled base64 decoder duplicates `mbedtls_base64_decode` (deliberate - keeps the lib mbedtls-free for host-testability, see `ota_signature.h`); the full-partition SHA-256 re-read was already accepted in design.md Decision 9/Risks; `mbedtls_pk_parse_public_key()` re-parsing the compiled-in key every attempt is real but negligible next to a multi-MB download+hash; `_otaCurrentSignature`/`_otaCurrentSignatureLen` having no mutex matches the existing task-lifecycle-sequenced pattern already used for `_otaCurrentUrl`/`_otaCurrentJobId`; `esp_https_ota_is_complete_data_received()` misreporting a complete download as incomplete needs a response with neither Content-Length nor chunked encoding, which this device's presigned-S3-only download source never produces
- [x] 15.6 Re-verified after fixes: `pio run -e esp32s3-dev -e esp32s3-prod` both SUCCESS; `pio test -e native` (WSL) 554/554 passed
