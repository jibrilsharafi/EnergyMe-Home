...
**1. Move `_compareVersions` to `utils.cpp`/`utils.h`, unchanged signature and semantics; underlying algorithm extracted to `lib/version_compare` for native unit test coverage.**
`int compareVersions(const char* current, const char* available)` - keep return convention (positive: current > available, 0: equal, negative: current < available) so `customserver.cpp`'s existing `_compareVersions(FIRMWARE_BUILD_VERSION, tagName) >= 0` call site needs only a rename, not a logic change. `utils.cpp`'s `compareVersions` is now a one-line delegate to `VersionCompare::compare` (`lib/version_compare/`), since `utils.cpp` itself pulls in Arduino/ESP-IDF headers and can't be compiled by the host (`native`) test environment - moving the parsing/comparison logic into a dependency-free `lib/` module (matching the existing convention used by `duration_format`, `phase_utils`, etc.) is what makes it possible to actually unit-test this function at all, rather than relying on manual/on-device verification only. Alternative considered: duplicate a small helper in `mqtt.cpp` instead of moving - rejected, the issue explicitly calls for one shared implementation and utils.cpp already hosts `isBackupVersionCompatible` as the version-logic home.
Added defensively: the parser now null-checks its input (a null `current`/`available` degrades to "0.0.0" instead of dereferencing a null pointer) - both current call sites already null-guard before calling, so this was unreachable in practice, but a shared utility shouldn't rely on every future caller remembering to do that.
...
- [Moving `_compareVersions` changes its access from private/static in `customserver.cpp` to a shared symbol in `utils.h`] -> Low risk: purely a code-motion refactor, no behavior change at the two existing call sites (update-check endpoint, and the new job-execution guard); confirmed via native tests (`test_version_compare`) and a clean `esp32s3-dev` build after the move.
- [A shared utility with no input validation is one missed null-check away from a crash at some future call site] -> Mitigated by adding the null-guard directly in `VersionCompare::compare` rather than relying on every caller to check first; covered by dedicated null-safety tests (`test_null_current_is_safe`, `test_null_available_is_safe`, `test_both_null_is_safe`).
...
- [`sscanf("%d", ...)` invokes undefined behavior on an out-of-range numeric component (e.g. `firmware.version: "99999999999.0.0"`), and the original subtraction-based compare (`currentMajor - availableMajor`) could itself overflow `int`] -> Found in a second-pass review ("is this thoroughly tested, are we sure it handles all strings"). Fixed by replacing `sscanf` with `strtol` (defined overflow behavior) clamped to `[0, INT_MAX]`, and replacing subtraction with an explicit three-way compare that cannot wrap. Covered by `test_overflow_major_clamps_not_wraps`, `test_int_max_boundary_no_overflow`, `test_negative_major_clamped_to_zero`.
- [Only lowercase `v` was stripped as a version prefix; a cloud tag like `"V2.2.0"` would parse as `0.0.0` and be misread as a downgrade] -> Fixed by stripping both `v` and `V`. Covered by `test_capital_v_prefix_not_treated_as_downgrade`.
- [A missing or non-string `firmware.version` made `targetVersion` null, which skipped the guard entirely (`if (!force && targetVersion)`) and let the job proceed unchecked - the opposite of guard-by-default] -> Changed to reject (`REJECTED`/`invalid_version`) when `!force` and `targetVersion` is null, closing the bypass. `force: true` remains the documented way to run a version-less job intentionally.
- [ArduinoJson's `as<bool>()` returns `true` for *any* non-null JSON value regardless of type (documented v6.12+ behavior, confirmed against ArduinoJson 7.4.2 docs) - a job document with `force` sent as the JSON string `"false"` would convert to `bool true` via `doc[...]["force"] | false`, silently bypassing the entire guard] -> Fixed by reading `force` as `variant.is<bool>() ? variant.as<bool>() : false`, so only an actual JSON boolean can set it; any other type (string, number, object) defaults to `false`. Covered by e2e case 3.6 (real AWS Job with `force: "false"` as a string).
