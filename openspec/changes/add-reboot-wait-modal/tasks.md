...
- [ ] 6.6 Bound the 90 s cap exactly: `startTime` is taken after the initial 900 ms delay and the loop only checks the cap before each poll (2.5 s poll timeout + 2 s interval), so the fallback can appear ~3 s past 90 s after the trigger.
