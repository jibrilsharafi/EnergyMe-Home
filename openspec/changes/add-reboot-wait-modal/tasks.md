...
- [ ] 6.5 Make the page under the overlay keyboard-inert: the scrim blocks pointer input only, Tab can still reach and activate controls on the underlying page (needs `inert` on the page content or a focus trap in the modal, plus `aria-modal`).
- [ ] 6.6 Bound the 90 s cap exactly: `startTime` is taken after the initial 900 ms delay and the loop only checks the cap before each poll (2.5 s poll timeout + 2 s interval), so the fallback can appear up to ~5 s past 90 s after the trigger (0.9 s pre-delay + 2.5 s poll timeout + 2 s interval).
