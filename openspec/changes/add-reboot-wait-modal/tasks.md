...
- [x] 6.6 90 s cap bounded exactly: the deadline is taken at `show()` (before the 900 ms pre-delay) and every poll timeout and interval is clamped to the time left, so the fallback appears at the cap (unit-tested with a virtual clock)
