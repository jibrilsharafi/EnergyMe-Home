...
- [x] 5.3 Confirm the upload failure path (non-success response) is unaffected and still shows its existing error messaging without invoking the wait screen. Correction: the original check missed a bug - `update.html` treated `data.success || data.message` as success, so a failed finalize (HTTP 200 with `success:false` and a message) showed "Update successful" and opened the reboot wait. Fixed in 01d5ff1: only `success === true` counts as success.
...
- [x] 6.3 Code review + simplification pass per this repo's PR checklist before merging to `development`. Adversarial review of `reboot-wait.js` found no real bugs (polling state machine, no-cors semantics, redirect-target safety, timer lifecycle, and OTA hand-off all checked out clean); the one real finding was spec.md/proposal.md/design.md still requiring filesystem restore to use the wait screen when the implementation and tasks.md correctly exclude it - fixed by updating the openspec docs to match. Simplification pass deduped a repeated `clearTimeout` call in `_pollHealth()`; kept all why-comments (no-cors rationale, failure-before-success gate); `pio run -e esp32s3-dev` confirmed no regressions.
- [x] 6.4 "Check again" dead-end fixed (febb92c): the fallback's retry re-ran the poll loop with the went-down gate, so a device that was already back (every poll succeeds) could never redirect. `show()` gained a `requireFailureFirst` option (default true); the retry passes `false`. Covered by `source/test/js/reboot_wait.test.js` (redirect after down/up, no redirect for a device that never went down, check-again redirect for a device already back, retry button re-polls without the gate).
- [x] 6.5 Page under the overlay is keyboard-inert: every other `body` child gets `inert` when the screen mounts (it is never dismissed, it ends in a navigation), the box is a `role="dialog"` `aria-modal` that takes focus, and the status line is `aria-live`. Verified in headless Chrome: Tab cycles only the dialog controls
- [x] 6.6 90 s cap bounded exactly: the deadline is taken at `show()` (before the 900 ms pre-delay) every poll timeout and interval is clamped to the time left, and no poll shorter than 1 s is started, so the fallback appears within the last second before the cap (unit-tested with a virtual clock)
