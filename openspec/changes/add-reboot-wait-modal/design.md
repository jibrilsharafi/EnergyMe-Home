...
**WiFi credential switch polls `energyme.local` as a best-effort target, same as network-config-apply's DHCP branch.**
Unlike the static-IP case, switching WiFi networks gives no way to know the device's new address in advance, the same problem factory reset has. The difference from factory reset: mDNS may still resolve if the browser's machine ends up on the same target network (a device moving from a guest network to the main one, say), where factory reset's SoftAP genuinely never will. Rather than a third special case, this reuses exactly the DHCP-branch target logic already in `configuration.html`'s network-config-apply flow. If the networks don't overlap, the 90s cap and manual fallback cover it, no worse than today's "wait 15 seconds and hope" with an unrecoverable disabled button.
...
- [The device sends no `Access-Control-Allow-Origin` header, so a cross-origin poll (static IP / DHCP / WiFi-switch targets) would have every request rejected by the browser's CORS check regardless of whether the device is actually up, permanently masking "back online" as "unreachable"] → Poll with `fetch(..., { mode: 'no-cors' })`. The response becomes opaque either way, but reachability is all the poll needs (the health endpoint has no failure response to distinguish, only up or unreachable), so losing read access to the response body costs nothing.
