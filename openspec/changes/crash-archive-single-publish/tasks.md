...
- [x] 9.7 v5 hardware profile kept permanently, not reverted as originally planned: the bench board is genuinely v5.0 hardware, not a stand-in for v6.x, so `esp32s3-dev-v5` and its `HardwareProfile` entry stay in the tree as a supported profile rather than throwaway scaffolding. The `parsePcbRevision()` leniency for a bare `vMAJOR` was reverted and stays reverted - `pcb_revision` must be the full `vMAJOR.MINOR` form. The bench board's factory NVS had `pcb_revision = "v5"` (malformed, silently falling into community mode) and a stale AWS IoT cert/key pair; both were corrected via the new NVS debug endpoint (see the `dev-nvs-management-endpoint` change) to `"v5.0"` and a freshly issued cert, which is what let the archived crash records from hardware verification actually publish and drain (8.10).
