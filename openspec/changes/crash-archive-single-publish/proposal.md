...
- **Archive core dumps to LittleFS at boot, then erase the partition.** On detection, capture the summary metadata, gzip the raw ELF dump, and write `/crashes/<crashId>_<elfSha256>.json` + `.bin.gz`. Retention: 10 records / 500 kB, oldest evicted first. The partition is erased only after both files are durably written.
