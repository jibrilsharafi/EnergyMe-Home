...
- [x] 8.1 Add a pure, host-testable decision helper (`lib/`) taking `(nowUnixSecond, targetBoundaryUnixSecond, deadlineSeconds, allChannelsFreshSinceBoundary) -> bool`, following the `lib/mqtt_grid_schedule` pattern. Added `lib/mqtt_energy_publish_gate`.
- [x] 8.2 Add `MQTT_ENERGY_PUBLISH_DEADLINE_SECONDS` (default 10s) to `source/include/mqtt.h`.
- [x] 8.3 In `source/src/mqtt.cpp`, add a helper that checks every active channel with valid measurements (plus the base-phase voltage read, channel 0) has `lastUnixTimeMilliseconds >= boundaryUnixMs`. Added `_allActiveChannelsFreshSinceBoundary()`.
- [x] 8.4 Update `_checkIfPublishEnergyNeeded()` to use the new gate: once `now >= boundary`, hold the publish (re-checking each pass) until all-channels-fresh or the deadline elapses, instead of firing immediately.
- [x] 8.5 Unit test the pure decision helper (before boundary; at boundary + fresh; at boundary + not fresh + before deadline; at/past deadline + not fresh) via `pio test -e native` (WSL). 9 new test cases, 212/212 total passing.
- [x] 8.6 Re-verify on dev bench device (OTA + UDP log capture): confirm every channel's `unixTime` in a published energy snapshot is now `>= ` the nominal boundary (no more pre-boundary stale points mixed into a post-boundary snapshot); confirm no busy-loop/regression versus the 6.1-6.5 cadence checks. Verified on .174: 4 consecutive cycles fired consistently ~1s after each `:00` boundary (vs. instantly before), matching the last muxed channel (WDRR-serviced) crossing the boundary; no deadline hits (well under the 10s cap); meter/grid topics and cadence unaffected; zero WARN/ERROR/FATAL.
