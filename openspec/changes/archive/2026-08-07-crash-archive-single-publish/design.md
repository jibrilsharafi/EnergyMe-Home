...
Two counting details are load-bearing and were both wrong in the first draft. First, the scan bound is expressed in *directory entries*, not records: a record is two files, so bounding the walk at the record count made every record past the halfway point invisible, and since retention only ever saw half the archive, neither cap could fire and `/crashes/` grew without limit. `CRASH_ARCHIVE_SCAN_FILES_MAX` is therefore derived as `2 * CRASH_ARCHIVE_SCAN_MAX + 4`, and `CRASH_ARCHIVE_SCAN_MAX` from `CRASH_ARCHIVE_MAX_RECORDS`, so none of the three can drift apart. Second, the byte total counts both files, since the cap is described as a ceiling on the directory; the sidecar is stat-ed after the walk rather than picked up during it, because the walk cannot rely on visiting a sidecar after its own dump file.
...
**6. One crash per publish cycle, re-armed on an interval like every other publisher.**
A backlog of 10 records is ~200 kB of back-to-back publishes; draining it in one task slice would delay meter and log publishing. Publishing one record per cycle keeps each slice bounded and lets normal telemetry interleave.

The re-arm is `_checkIfPublishCrashNeeded()`, alongside the other `_checkIfPublish*Needed()` calls, rather than the publisher setting its own flag from its success path. Self-scheduling was the original shape and it had one failure mode: every failure path cleared the flag to avoid a tight retry loop, and nothing else ever set it again, so a single transient failure stopped all crash publishing until the next reboot. On an interval a failure costs one interval. The interval check runs before `hasArchivedCrash()`, since that walks a LittleFS directory and this is on every MQTT tick.

Publishing also distinguishes *retry* from *skip*. A record rejected by the size guard (decision 5) can never become publishable, so retrying it parks it at the head of the queue forever and no later crash ever ships. Those are stepped over, bounded per cycle; the record stays on flash for retrieval over the local API, which is what the error log tells the operator to do.
...
The budget belongs to the dump currently on the partition, so it is also cleared on any boot that finds none pending. Without that it leaks across dumps: the semaphore timeout path returns with an attempt already spent while the orphaned task keeps running, and if that task then succeeds nothing reads its result. Two such timeouts would leave the counter at 2, and an unrelated crash three boots later would get one attempt and then be discarded on sight.
