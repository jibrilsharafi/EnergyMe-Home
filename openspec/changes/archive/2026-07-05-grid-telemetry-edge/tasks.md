...
- [x] 4.3 Bench test: points land within a few ms of .000/.500, gaps appear when flag off / line events; commit `feat(grid): 500ms wall-clock-aligned grid sampler task`
...
- [x] 5.3 Bench test flag flip via shadow (cloud -> device -> persisted across reboot); commit `feat(mqtt): cloud-settable send_grid_data flag`
...
- [x] 6.3 Bench e2e (.174): enable flag, verify batched arrays arrive on the grid topic via MQTT test client, meter payload byte-identical to before; commit `feat(mqtt): dedicated batched grid telemetry topic`

## 7. Verification + handoff

- [x] 7.1 Soak test on .174 (>=1 h): heartbeats, heap, `getDebugCount()` rate, energy continuity vs. pre-change baseline
- [x] 7.2 Confirm `infra-handoff.md` names/schema match the shipped firmware literals; hand to energyme-infra
- [x] 7.3 Open PR to `development` with `Closes #157`
