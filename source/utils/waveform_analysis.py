...
class WaveformAnalyzer:
    ...
    @classmethod
    def analyze(cls, waveform: WaveformData) -> ComputedProperties:
        ...
        # THD
        v_thd = cls.compute_thd(voltage, frequency, float(sample_rate))
        ...
        # Reactive power (from power triangle)
...
def plot_waveforms(waveform: WaveformData, computed: ComputedProperties, 
    ...
    fig.suptitle(f"{channel_label} - Waveform Analysis", fontsize=16, fontweight='bold')
...
def main():
    ...
    # Configuration
    BASE_URL = "http://192.168.1.68"  # Change to your meter's IP
    CHANNEL_INDEX = 1  # Change to desired channel
    
    # Initialize API client
    api = EnergyMeterAPI(BASE_URL)
    
    try:
        # Get channel configuration
        print(f"\nFetching channel {CHANNEL_INDEX} configuration...")
        channel_config = api.get_channel_config(CHANNEL_INDEX)
        print(f"Channel: {channel_config.get('label', 'Unknown')}")
        print(f"Active: {channel_config.get('active', False)}")
        
        # Capture waveform
        waveform = api.capture_waveform_blocking(CHANNEL_INDEX, timeout=15)
        print(f"\nCaptured {waveform.sample_count} samples")
        
        # Get meter values for comparison
        print("\nFetching meter values...")
        meter_values = api.get_meter_values(CHANNEL_INDEX)
        ...
        plt.savefig(f'temp/waveform_analysis_ch{CHANNEL_INDEX}.png', dpi=150, bbox_inches='tight')
        print(f"Plot saved as 'temp/waveform_analysis_ch{CHANNEL_INDEX}.png'")
        plt.show()
